...
class StablePPOController:
    ...
    def save_models(self, path_prefix):
        """保存模型"""
        if "models7" not in path_prefix:
            path_prefix = path_prefix.replace("models/", "models7/").replace("model2/", "models7/")
        
        os.makedirs(os.path.dirname(path_prefix) if os.path.dirname(path_prefix) else "models7", exist_ok=True)
        for i in range(self.n_agents):
...
def train_stable_ppo(
    ...
    for episode in range(n_episodes):
        ...
        if collective_reward > best_collective_reward:
            ...
            controller.save_models(f"models7/best_{experiment_name}")
        ...
        if episode % 10000 == 0 and episode > 0:
            controller.save_models(f"models7/{experiment_name}_ep{episode}")
    
    # 最终保存
    controller.save_models(f"models7/{experiment_name}_final")
    ...
    os.makedirs('models7', exist_ok=True)
    with open(f"models7/results_{experiment_name}.json", 'w') as f:
        json.dump(results, f)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="稳定版目标导向多智能体PPO")
    ...
    parser.add_argument("--no-wandb", action="store_true", 
                        help="禁用WandB日志")
    parser.add_argument("--project", type=str, default="stable-goal-focused-ppo100k", 
                        help="WandB项目名称")
    ...
    print(f"\n✅ 稳定版训练完成!")
    print(f"📁 模型已保存到: models7/{experiment_name}_final")
    print(f"📊 结果已保存到: models7/results_{experiment_name}.json")